...
const BrandLogo = () => (
  <svg viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-8 h-8 md:w-9 md:h-9 drop-shadow-[0_0_15px_rgba(234,179,8,0.3)] shrink-0 transition-transform group-hover:scale-105 transform-gpu">
...
export function Navbar() {
  ...
  useEffect(() => {
    // Usando passive listener e um threshold levemente maior para evitar disparos desnecessários
    const handleScroll = () => {
  ...
  return (
    <>
      {/* 
        Aceleração via GPU (transform-gpu, will-change-auto).
        A Navbar não altera mais seu 'width' e sim a cor e o padding da 'inner box'.
      */}
      <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ease-out transform-gpu will-change-auto pointer-events-none ${scrolled ? 'p-3 md:p-4' : 'p-0'}`}>
        ...
        <div className={`pointer-events-auto mx-auto w-full max-w-6xl flex items-center justify-between transition-all duration-300 ease-out transform-gpu ${
          scrolled 
            ? "bg-[#050505]/90 backdrop-blur-lg border border-white/10 rounded-2xl px-6 md:px-8 py-3.5 shadow-2xl" 
            : "bg-[#030303] border-b border-white/5 px-6 md:px-8 py-5 rounded-none max-w-none"
        }`}>
          
          {/* LOGO VETORIAL E NOME */}
          <Link href="/" className="flex items-center gap-3 group transform-gpu">
            <BrandLogo />
            <span className="text-base md:text-lg font-black tracking-widest text-white uppercase italic group-hover:text-yellow-400 transition-colors">
              Pay<span className="text-yellow-400 group-hover:text-white transition-colors">MTA</span>
            </span>
          </Link>
          ...
          <div className="flex items-center gap-3">
            <Link 
              ...
              className="hidden sm:flex items-center gap-2 px-4 py-2.5 bg-yellow-400 hover:bg-yellow-500 text-black font-black text-xs uppercase tracking-wider rounded-xl transition-all shadow-lg shadow-yellow-400/20 active:scale-95 transform-gpu"
            >
      ...
      {/* MENU MOBILE (CSS Nativo, sem framer-motion para máxima performance) */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-40 bg-[#030303]/95 backdrop-blur-xl pt-28 px-6 md:hidden flex flex-col justify-between pb-12 animate-in fade-in slide-in-from-top-4 duration-200 transform-gpu">
          ...
          <div className="flex flex-col gap-4">
            <Link 
              href="/api/auth/signin" 
              onClick={() => setMobileMenuOpen(false)}
              className="flex items-center justify-center gap-2 w-full py-4 bg-yellow-400 text-black font-black text-sm uppercase tracking-wider rounded-2xl shadow-lg shadow-yellow-400/20 active:scale-95 transition-transform transform-gpu"
            >
