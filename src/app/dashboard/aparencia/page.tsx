...
export default function AppearancePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) setToast({ message: "Design atualizado com sucesso!", type: "success" });
    } catch (error) {
  ...
  if (fetching) return (
    <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
      ...
      <span className="text-zinc-500 font-black uppercase italic text-[10px]">Sincronizando Identidade...</span>
    </div>
  );
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <div className="flex items-center gap-2 text-yellow-500 mb-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.3em]">Branding & Marketing</span>
          </div>
          <h1 className="text-4xl font-black italic uppercase tracking-tighter text-white">Design da Loja</h1>
          <p className="text-zinc-500 text-sm font-medium mt-1">Personalize a experiência visual e a mensagem da sua marca.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-8">
        ...
        <section className="bg-zinc-950/50 border border-white/5 p-10 rounded-[40px] shadow-2xl backdrop-blur-sm">
          <div className="flex items-center gap-4 mb-8">
            ...
            <h3 className="text-xl font-black italic uppercase text-white leading-tight">Cores e Identidade</h3>
          </div>
          <div className="flex flex-col md:flex-row items-center gap-10 p-8 bg-black/40 rounded-[32px] border border-white/5">
            ...
            <div className="flex-1">
              <p className="text-[10px] font-black uppercase text-zinc-500 mb-1">Cor Principal (HEX)</p>
              <p className="text-4xl font-mono font-black text-white uppercase italic">{form.primaryColor}</p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-10">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-zinc-500 uppercase ml-1">URL da Logo (PNG)</label>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-zinc-500 uppercase ml-1">URL do Banner Hero</label>
