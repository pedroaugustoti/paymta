...
export default function AppearancePage() {
  ...
  // CARREGAMENTO DOS DADOS (Sincronização com o Supabase)
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) alert("✅ Design atualizado com sucesso!");
    } catch (error) {
            ...
            <div className="flex-1 text-center md:text-left space-y-4">
              ...
              <div className="flex items-start gap-3 p-4 bg-purple-500/5 rounded-2xl border border-purple-500/10">
                ...
                <p className="text-[11px] text-zinc-400 leading-relaxed font-medium">
                  <strong>Onde essa cor aparece?</strong> Ela será aplicada automaticamente em todos os botões de compra, ícones de destaque, barras de progresso e elementos interativos da sua loja pública.
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="bg-zinc-950/50 border border-white/5 p-10 rounded-[40px] shadow-2xl backdrop-blur-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div className="space-y-4">
              ...
              <p className="text-[10px] text-zinc-500 leading-relaxed px-1">
                Este logo aparecerá no canto superior esquerdo do site. Se deixado em branco, o sistema exibirá o nome do servidor em texto.
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest ml-1">Banner Principal (Hero)</label>
              ...
              <p className="text-[10px] text-zinc-500 leading-relaxed px-1">
                A imagem de fundo que recebe os jogadores. Use fotos do seu servidor ou artes temáticas para causar impacto.
              </p>
            </div>
          </div>
          ...
          <div className="mt-12 p-6 bg-white/5 rounded-[32px] border border-white/5 flex flex-col md:flex-row items-center gap-8">
             <div className="w-full md:w-48 h-28 bg-black rounded-2xl overflow-hidden border border-white/10 relative">
                {form.heroImageUrl ? (
                   <img src={form.heroImageUrl} className="w-full h-full object-cover opacity-50" />
                ) : <div className="w-full h-full flex items-center justify-center text-zinc-800 italic text-[10px]">Sem Banner</div>}
                <div className="absolute top-4 left-4">
                  {form.logoUrl ? <img src={form.logoUrl} className="h-4 w-auto" /> : <div className="h-4 w-12 bg-white/10 rounded-full" />}
                </div>
             </div>
             <div className="flex-1">
                ...
                <p className="text-[10px] text-zinc-500 font-medium leading-relaxed">
                  As imagens acima são links diretos. Recomendamos o uso do <strong>Imgur</strong> ou o próprio <strong>Discord</strong> para hospedar suas imagens e garantir que elas carreguem rápido na Vercel.
                </p>
             </div>
          </div>
        </section>
