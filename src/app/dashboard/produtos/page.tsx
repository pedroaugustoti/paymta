...
export default function ProductsPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "Erro ao processar pacote. Verifique os dados.";
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-10 animate-in fade-in duration-700 relative">
      ...
      <PageHeader 
        title="Produtos"
        subtitle="Gerencie o inventário da loja, preços e categorias."
        ...
        actionButton={{
          ...
          colorClass: "bg-white hover:bg-zinc-200 text-black shadow-xl"
        }}
      />

      <SectionCard 
        title="Inventário" 
        subtitle="Todos os itens ativos na loja"
        ...
        headerAction={
          <div className="relative w-full md:w-64">
            ...
            <input 
              ...
              className="w-full bg-zinc-900 border border-white/5 rounded-2xl py-3 pl-12 pr-4 text-xs focus:border-yellow-500 outline-none text-white transition-colors"
            />
          </div>
        }
      >
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProducts.map((product) => (
            <div key={product.id} className="bg-black/40 border border-white/5 rounded-[32px] overflow-hidden group hover:border-yellow-500/30 transition-all shadow-2xl">
                ...
                <div className="flex items-center justify-between pt-6 border-t border-white/5">
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      className="p-3 bg-white/5 rounded-xl hover:bg-white/10 text-zinc-400 hover:text-white transition-all cursor-pointer"
                    >
                      <Edit3 className="w-4 h-4" />
                    </button>
                    <button 
                      onClick={() => handleDelete(product.id)}
                      className="p-3 bg-red-500/5 rounded-xl hover:bg-red-500/20 text-red-900 hover:text-red-500 transition-all cursor-pointer"
                    >
            ...
            >
              <div className="p-10">
                <div className="flex justify-between items-center mb-10">
                  ...
                  <button onClick={() => setIsModalOpen(false)} className="p-3 bg-white/5 hover:bg-white/10 rounded-full text-zinc-400 hover:text-white transition-colors cursor-pointer">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                ...
                <form onSubmit={handleSave} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <input required type="text" value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="w-full bg-black border border-white/10 rounded-2xl p-4 text-sm focus:border-yellow-500 outline-none text-white font-bold transition-colors" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-zinc-500 uppercase ml-1">Preço (R$)</label>
                      <input required type="number" step="0.01" value={form.price} onChange={e => setForm({...form, price: e.target.value})} className="w-full bg-black border border-white/10 rounded-2xl p-4 text-sm focus:border-emerald-500 outline-none text-white font-mono transition-colors" />
                    </div>
                  </div>

                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-zinc-500 uppercase ml-1">Categoria na Loja</label>
                    <select value={form.category} onChange={e => setForm({...form, category: e.target.value})} className="w-full bg-black border border-white/10 rounded-2xl p-4 text-sm focus:border-yellow-500 outline-none text-zinc-300 cursor-pointer transition-colors">
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea rows={3} value={form.description} onChange={e => setForm({...form, description: e.target.value})} className="w-full bg-black border border-white/10 rounded-2xl p-4 text-sm focus:border-yellow-500 outline-none text-zinc-300 min-h-[80px] transition-colors resize-none" />
                  </div>

                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-zinc-500 uppercase ml-1">URL da Imagem Ilustrativa</label>
                    <input type="text" value={form.image} onChange={e => setForm({...form, image: e.target.value})} className="w-full bg-black border border-white/10 rounded-2xl p-4 text-sm focus:border-purple-500 outline-none text-white transition-colors" />
                  </div>

                  <div className="pt-6">
                    <button type="submit" className="w-full bg-yellow-500 hover:bg-yellow-400 text-black font-black py-8 rounded-[24px] text-lg shadow-xl shadow-yellow-500/10 uppercase italic transition-all active:scale-95 cursor-pointer">
