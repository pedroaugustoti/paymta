  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setToast({ message: "Falha ao salvar conteúdo.", type: "error" });
      }
    } catch (error) {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 relative">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-white/5 pb-10">
        ...
        <Button onClick={handleSave} disabled={loading} className="bg-emerald-600 hover:bg-emerald-700 text-white font-black px-10 py-7 rounded-2xl flex items-center gap-2 shadow-xl shadow-emerald-500/10 transition-all active:scale-95 cursor-pointer">
      ...
      <div className="grid grid-cols-1 gap-10">
        <section className="bg-zinc-950/50 border border-white/5 p-10 rounded-[40px] shadow-2xl backdrop-blur-sm">
          <div className="flex items-center gap-4 mb-8">
            <div className="p-3 bg-emerald-500/10 rounded-2xl text-emerald-500"><ShieldCheck className="w-6 h-6" /></div>
            <div>
          ...
          <div className="space-y-6">
            ...
            <textarea rows={12} value={form.termsContent} onChange={(e) => setForm({...form, termsContent: e.target.value})} placeholder="Ex: Ao adquirir créditos em nossa loja, você concorda que..." className="w-full bg-black border border-white/10 rounded-[32px] p-8 text-sm focus:border-emerald-500 outline-none text-zinc-300 leading-relaxed font-medium min-h-[400px] scrollbar-hide" />
          </div>
        </section>
        ...
        <section className="bg-zinc-950/50 border border-white/5 p-10 rounded-[40px] shadow-2xl backdrop-blur-sm opacity-60">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-blue-500/10 rounded-2xl text-blue-500"><ListOrdered className="w-6 h-6" /></div>
              <div>
